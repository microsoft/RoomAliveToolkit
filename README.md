...
- Projectors and cameras must be arranged so that the calibration procedure can infer the overall arrangement of all cameras and projectors. In practice, this means that some Kinect sensor's color and depth cameras must observe the projection from mulitple projectors. For example, in a configuration with two projectors and one camera, the camera must observe parts of both projections. In a configuration with three projectors and two cameras, it will suffice for the first camera to observe parts of the first and second projectors' images, and the second camera to observe parts of the second and third projectors' images. Esstentially, the cameras not only provide the information necessary to solve for projector camera intrinsics, they also provide the intermediate coordinate systems necessary to find the relative pose of the projectors. It is not necessary for the projections to overlap. We use the term ‘ensemble’ to denote the set of cameras and projectors that are calibrated together. We refer to a projector and the set of cameras that can see some part of the projector’s image as a ‘projector group’. An ensemble is thus a set of projector groups that share cameras.
